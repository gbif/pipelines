...
/**
 * Presents the DwCA star records as ExtendedRecords. The core record ID will be used or if not
 * present, a UUID is created.
 */
  ...
  static class ExtendedRecordReader extends RecordReader<Text, ExtendedRecord> {
    ...
    public void initialize(InputSplit inputSplit, TaskAttemptContext context) throws IOException {
      ...
      java.nio.file.Path tmpFile =
          Files.createTempFile("tmp", ".zip"); // consider using job and task IDs?
      ...
      // having copied the file out of HDFS onto the local FS in a temp folder, we prepare it (sorts
      // files)
      java.nio.file.Path tmpSpace =
      ...
      reader =
          new DwCAReader(tmpFile.toAbsolutePath().toString(), tmpSpace.toAbsolutePath().toString());
      nextKeyValue();
    }
