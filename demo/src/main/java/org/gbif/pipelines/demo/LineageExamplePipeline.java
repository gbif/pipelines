...
public class LineageExamplePipeline {
  ...
  public static void main(String[] args) {
    ...
    PCollectionTuple results =
      rawRecords.apply(ParDo
      .of(new DoFn<ExtendedRecord, UntypedOccurrence>() {
      ...
      .withOutputTags(
        data, TupleTagList.of(lineage)
      ));


    // Write the data result as an Avro file
    results.get(data).apply(
    ...
    PCollection<String> lineageAsJSON = finalLineage.apply(ParDo.of(new DoFn<KV<String,Iterable<String>>, String>() {
      ...
      public void processElement(ProcessContext c) {
        ...
        StringBuilder recordAsJSON = new StringBuilder("{\n" +
                              "  \"key\":" + record.getKey() + "\n" +
                              "  \"lineage\": [\n");
    ...
    lineageAsJSON.apply(
      "Save the records as Avro", TextIO.write().to("demo/output/lineage.json")); // not valid JSON but for demo...
