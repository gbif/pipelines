...
/** Test class for {@link TaggedValuesInterpreter} */
public class TaggedValueInterpreterTest {
  ...
  private void genericTaggedValueTest(String tagName, Term fromTerm, Term toTerm) {
    ...
    MachineTag machineTag = MachineTag.newBuilder()
      .setNamespace(TaggedValuesInterpreter.PROCESSING_NAMESPACE)
      .setName(tagName)
      .setValue(key + ":" + testValue)
      .build();
    MetadataRecord mr = MetadataRecord.newBuilder()
      .setId("1")
  ...
  public void testCollectionTaggedValues() {
    ...
    genericTaggedValueTest(TaggedValuesInterpreter.COLLECTION_TAG_NAME,
                           DwcTerm.collectionCode,
                           GbifInternalTerm.collectionKey);

    genericTaggedValueTest(TaggedValuesInterpreter.INSTITUTION_TAG_NAME,
                           DwcTerm.institutionCode,
                           GbifInternalTerm.institutionKey);

    genericTaggedValueTest(TaggedValuesInterpreter.COLLECTION_TO_INSTITUTION_TAG_NAME,
                           DwcTerm.collectionCode,
                           GbifInternalTerm.institutionKey);

    genericTaggedValueTest(TaggedValuesInterpreter.INSTITUTION_TO_COLLECTION_TAG_NAME,
                           DwcTerm.institutionCode,
                           GbifInternalTerm.collectionKey);
  }
