 ...
 *
 * Code copied from TemporalInterpreter and converted static methods to none-static methods.
 *
 * */
...
public class DefaultTemporalInterpreter implements Serializable{
  ...
  private DefaultTemporalInterpreter(){
   ...
   * @return
   */
  public static DefaultTemporalInterpreter getInstance(DateComponentOrdering[] orderings) {
    return new DefaultTemporalInterpreter(orderings);
  }
    ...
    if (hasValue(er, DwcTerm.georeferencedDate)) {
      ...
      OccurrenceParseResult<TemporalAccessor> parsed =
          interpretLocalDate(
              ...
              validRecordedDateRange,
              OccurrenceIssue.GEOREFERENCED_DATE_UNLIKELY);
      if (parsed.isSuccessful()) {
        Optional.ofNullable(
            TemporalAccessorUtils.toEarliestLocalDateTime(parsed.getPayload(), false))
            .map(LocalDateTime::toString)
      ...
      addIssueSet(lr, parsed.getIssues());
    } else {
      addIssue(lr, OccurrenceIssue.GEOREFERENCED_DATE_INVALID.name());
    }
  }
