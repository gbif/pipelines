...
@Builder
public class LoanTableTransform implements Serializable {

  @NonNull private final TupleTag<ExtendedRecord> extendedRecordTag;
  @NonNull private final TupleTag<BasicRecord> basicRecordTag;

  public ParDo.SingleOutput<KV<String, CoGbkResult>, LoanTable> converter() {
    DoFn<KV<String, CoGbkResult>, LoanTable> fn =
        new DoFn<KV<String, CoGbkResult>, LoanTable>() {

          private final Counter counter =
              Metrics.counter(LoanTableTransform.class, LOAN_TABLE_RECORDS_COUNT);
          ...
          public void processElement(ProcessContext c) {
            ...
            LoanTableConverter.convert(br, er)
                .ifPresent(
                    record -> {
                      c.output(record);
                      counter.inc();
  ...
  public AvroIO.Write<LoanTable> write(String toPath, Integer numShards) {
    AvroIO.Write<LoanTable> write =
        AvroIO.write(LoanTable.class)
            .to(toPath)
