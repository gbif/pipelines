...
@Builder
public class ChronometricAgeTableTransform implements Serializable {

  @NonNull private final TupleTag<ExtendedRecord> extendedRecordTag;
  @NonNull private final TupleTag<BasicRecord> basicRecordTag;

  public ParDo.SingleOutput<KV<String, CoGbkResult>, ChronometricAgeTable> converter() {
    DoFn<KV<String, CoGbkResult>, ChronometricAgeTable> fn =
        new DoFn<KV<String, CoGbkResult>, ChronometricAgeTable>() {

          private final Counter counter =
              Metrics.counter(
                  ChronometricAgeTableTransform.class, CHRONOMETRIC_AGE_TABLE_RECORDS_COUNT);
          ...
          public void processElement(ProcessContext c) {
            ...
            ChronometricAgeTableConverter.convert(br, er)
                .ifPresent(
                    record -> {
                      c.output(record);
                      counter.inc();
  ...
  public AvroIO.Write<ChronometricAgeTable> write(String toPath, Integer numShards) {
    AvroIO.Write<ChronometricAgeTable> write =
        AvroIO.write(ChronometricAgeTable.class)
            .to(toPath)
