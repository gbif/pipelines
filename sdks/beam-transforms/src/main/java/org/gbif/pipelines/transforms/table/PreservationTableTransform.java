...
@Builder
public class PreservationTableTransform implements Serializable {

  @NonNull private final TupleTag<ExtendedRecord> extendedRecordTag;
  @NonNull private final TupleTag<BasicRecord> basicRecordTag;

  public ParDo.SingleOutput<KV<String, CoGbkResult>, PreservationTable> converter() {
    DoFn<KV<String, CoGbkResult>, PreservationTable> fn =
        new DoFn<KV<String, CoGbkResult>, PreservationTable>() {

          private final Counter counter =
              Metrics.counter(PreservationTableTransform.class, PRESERVATION_TABLE_RECORDS_COUNT);
          ...
          public void processElement(ProcessContext c) {
            ...
            PreservationTableConverter.convert(br, er)
                .ifPresent(
                    record -> {
                      c.output(record);
                      counter.inc();
  ...
  public AvroIO.Write<PreservationTable> write(String toPath, Integer numShards) {
    AvroIO.Write<PreservationTable> write =
        AvroIO.write(PreservationTable.class)
            .to(toPath)
