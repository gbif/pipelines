...
public class InterpretedToEsIndexExtendedPipelineIT {
  ...
  public void reindexingDatasetInSameDefaultIndexTest() {
    ...
    // Should
    assertEquals(DEFAULT_REC_DATASET, EsIndex.countDocuments(ES_SERVER.getEsConfig(), options.getEsIndexName()));
    assertEquals(DEFAULT_REC_DATASET, countDocumentsFromQuery(ES_SERVER, options.getEsIndexName(),
   ...
   * 2. Switch datasets to different indexes without changing the number of records.
   * 3. Switch datasets to different indexes and changing the number of records at the same time.
   */
  @Test
  public void switchingIndexTest() {
    ...
    List<String> allDatasets =
        ImmutableList.<String>builder().addAll(staticDatasets)
    ...
    attempt++;
    switchDatasetAndRecords(ImmutableList.of(DATASET_TEST, DATASET_TEST_9), DYNAMIC_IDX, attempt, 0);
    switchDatasetAndRecords(ImmutableList.of(DATASET_TEST_4, DATASET_TEST_7), STATIC_IDX, attempt, 0);
  ...
  private void switchDatasetAndRecords(List<String> datasets, String targetIdx, int attempt, int diffRecords) {
    datasets.forEach(datasetKey -> {
      // there should be only one source index
      String sourceIdx =
          EsIndex.findDatasetIndexesInAliases(ES_SERVER.getEsConfig(), new String[]{ALIAS}, datasetKey)
              .iterator()
              .next();
      ...
      // When
      indexDatasets(ES_SERVER, ImmutableList.of(datasetKey), attempt, targetIdx, ALIAS, recordsDataset);

      // Should
      assertEquals(previousCountAlias + diffRecords, EsIndex.countDocuments(ES_SERVER.getEsConfig(), ALIAS));
      assertEquals(previousTargetCount + recordsDataset, EsIndex.countDocuments(ES_SERVER.getEsConfig(), targetIdx));
