...
import org.gbif.pipelines.tasks.balancer.handler.PipelinesCamtrapDpMessageHandler;
...
public class BalancerCallback extends AbstractMessageCallback<PipelinesBalancerMessage> {
  ...
  public void handleMessage(PipelinesBalancerMessage message) {
    ...
    try {
      ...
      } else if (PipelinesCamtrapDpMessage.class.getSimpleName().equals(className)) {
        PipelinesCamtrapDpMessageHandler.handle(publisher, message);
      } else {
