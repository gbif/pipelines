...
| `event.eventProtocol_fk → protocol` | `dwc:samplingProtocol` | Uses `protocolDescription`. |
| `event-protocol → protocol` | `dwc:samplingProtocol` | Linked protocol descriptions are distinct, deterministically ordered, and merged with the direct protocol value. |
| `event.georeferenceProtocol_fk → protocol` | `dwc:georeferenceProtocol` | Uses `protocolDescription` only when the event's text field is null. |
...
| `survey.samplingProtocol[_fk]` | `eco:protocolDescriptions` | Supplied text wins; linked `protocolDescription` is a fallback. |
| `survey.samplingEffortProtocol[_fk]` | `eco:samplingEffortProtocol` | Supplied text wins; linked `protocolDescription` is a fallback. |
...
| `occurrenceProtocol_fk → protocol` | `dwc:samplingProtocol` | Uses `protocolDescription`. |
...
| `material-protocol → protocol` | `dwc:samplingProtocol` | Linked protocol descriptions are aggregated and merged with an occurrence protocol value. |
