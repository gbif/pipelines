...
public final class CompiledMappingDatasetPruner {
  ...
  private static CompiledCoreFragment pruneCoreFragment(
      ...
      Set<org.gbif.pipelines.spark.dwcdp.mapping.FieldRef> mergeFields) {
  ...
  private static CompiledFragment pruneFragmentTargets(
    ...
    Optional<org.gbif.pipelines.spark.dwcdp.mapping.FieldRef> rowIdentity =
        fragment.rowIdentity().filter(scope::supports);
    Optional<org.gbif.pipelines.spark.dwcdp.mapping.FieldRef> rowMatch =
  ...
  private static Set<org.gbif.pipelines.spark.dwcdp.mapping.FieldRef> mergeFieldsForOwner(
  ...
  private static Set<org.gbif.pipelines.spark.dwcdp.mapping.FieldRef> supportedFields(
      Set<org.gbif.pipelines.spark.dwcdp.mapping.FieldRef> fields, MappingDatasetScope scope) {
    Set<org.gbif.pipelines.spark.dwcdp.mapping.FieldRef> supported = new LinkedHashSet<>();
    fields.stream().filter(scope::supports).forEach(supported::add);
    return supported;
  }

  private static Set<org.gbif.pipelines.spark.dwcdp.mapping.FieldRef> targetFields(
      List<CompiledTargetProducer> targets) {
    Set<org.gbif.pipelines.spark.dwcdp.mapping.FieldRef> fields = new LinkedHashSet<>();
