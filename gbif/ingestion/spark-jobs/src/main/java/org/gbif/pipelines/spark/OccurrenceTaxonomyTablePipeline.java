...
public class OccurrenceTaxonomyTablePipeline {
  ...
  public static void main(String[] argsv) throws IOException {
    ...
    OccurrenceTaxonomyTablePipeline.Args args = new OccurrenceTaxonomyTablePipeline.Args();
    ...
    spark.sql(
        String.format(
            """
              DROP TABLE IF EXISTS %s.occurrence_taxonomy
            """,
            args.targetDB));
    ...
    spark.sql(
        String.format(
            """
              CREATE TABLE %s.occurrence_taxonomy (
                    gbifid STRING,
                ...
                USING iceberg
                PARTITIONED BY (checklistkey, datasetkey)
                TBLPROPERTIES (%s)
                """,
    ...
    // write to table
    occTaxDf.write().mode(SaveMode.Append).insertInto(args.targetDB + "." + "occurrence_taxonomy");
  ...
  static Dataset<Row> buildOccurrenceTaxonomy(Dataset<Row> occ) {
    return occ.select(
            col("gbifid"), col("classificationdetails"), col("taxonomicissue"), col("datasetkey"))
        .withColumn("entry", explode(map_entries(col("classificationdetails"))))
        .select(
            col("gbifid"),
            col("entry.key").alias("checklistkey"),
            col("entry.value").alias("classification"),
            col("taxonomicissue"),
            col("datasetkey"))
        .withColumn("taxonomicissue_", element_at(col("taxonomicissue"), col("checklistkey")))
        // key columns
        ...
        .select(
            ...
            col("taxonomicissue_").alias("taxonomicissue"),
            col("checklistkey"),
            col("datasetkey"));
  }
}
