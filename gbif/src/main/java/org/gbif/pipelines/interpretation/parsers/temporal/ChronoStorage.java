...
class ChronoStorage {
  ...
  static ChronoStorage from(String year, String month, String day) {
    ChronoStorage temporal = new ChronoStorage();
  ...
  ChronoStorage merge(ChronoStorage chronoStorage) {
    valueMap.putAll(chronoStorage.valueMap);
    if (chronoStorage.getLastParsed() != null) {
      lastParsed = chronoStorage.getLastParsed();
    }
    return this;
  }

  void putAll(ChronoStorage storage) {
    valueMap.putAll(storage.valueMap);
  }

  void putAllIfAbsent(ChronoStorage storage) {
