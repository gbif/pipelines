...
public final class MockGbifInterpretationPipeline {
  ...
  /**
   * Creates an interpretation {@link Pipeline} from {@link DataProcessingPipelineOptions} and {@link Config}.
   */
  public static Pipeline mockInterpretationPipeline(DataProcessingPipelineOptions options, Config wsConfig) {
  ...
  private static EnumMap<InterpretationType, InterpretationStepSupplier> createStepsMap(
    ...
    EnumMap<InterpretationType, InterpretationStepSupplier> stepsMap = new EnumMap<>(InterpretationType.class);
    CodecFactory avroCodec = CodecFactory.deflateCodec(CodecFactory.DEFAULT_DEFLATE_LEVEL);

    stepsMap.put(LOCATION, locationGbif(createPaths(options, InterpretationType.LOCATION), avroCodec, wsConfig));
    stepsMap.put(TEMPORAL, temporalGbif(createPaths(options, InterpretationType.TEMPORAL), avroCodec));
    stepsMap.put(TAXONOMY, taxonomyGbif(createPaths(options, InterpretationType.TAXONOMY), avroCodec, wsConfig));
