...
public class VerbatimMessageHandler {
  ...
  private static StepRunner computeRunner(BalancerConfiguration config, PipelinesVerbatimMessage message,
    ...
    String verbatimPath = String.join("/", config.stepConfig.repositoryPath, datasetId, attempt, verbatim);
    long fileSizeByte = HdfsUtils.getFileSizeByte(verbatimPath, config.stepConfig.hdfsSiteConfig);
    if (fileSizeByte > 0) {
  ...
  private static Integer getLatestAttempt(BalancerConfiguration config, PipelinesVerbatimMessage message) {
    String datasetId = message.getDatasetUuid().toString();
    String path = String.join("/", config.stepConfig.repositoryPath, datasetId);
    log.info("Parsing HDFS directory - {}", path);
    return HdfsUtils.getSubDirList(config.stepConfig.hdfsSiteConfig, path)
        .stream()
