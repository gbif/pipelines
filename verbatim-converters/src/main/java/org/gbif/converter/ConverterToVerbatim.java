...
import org.gbif.utils.FileSystemUtils;
...
public abstract class ConverterToVerbatim {
  ...
  public boolean convert(java.nio.file.Path inputPath, Path outputPath) {
    ...
    // that is being used
    // by another consumer.
    FileSystem fs = FileSystemUtils.createParentDirectories(outputPath, hdfsSiteConfig);
    try (BufferedOutputStream outputStream = new BufferedOutputStream(fs.create(outputPath));
      ...
      isConverted = FileSystemUtils.deleteAvroFileIfEmpty(fs, outputPath);
    }

    return !isConverted;
  }

  abstract void convert(java.nio.file.Path inputPath, DataFileWriter<ExtendedRecord> dataFileWriter)
      throws IOException;
}
