...
public class AvroToESJoinPipeline {
  private static final String INPUT = "hdfs://ha-nn/user/hive/warehouse/gbif-data/00ddc92e-fde1-4dda-8712-ff46582a5e6b/1/";
  ...
  public static void main(String[] args) {
    ...
    DataProcessingPipelineOptions options =
      DataPipelineOptionsFactory.create(args);
    ...
    pipeline.apply(AvroIO.read(Location.class).from(INPUT+"location/interpreted*.avro"))
      ...
      .apply(ElasticsearchIO.write()
               .withConnectionConfiguration(ElasticsearchIO.ConnectionConfiguration.create(ES_ADDRESSES,
                                                                                           ES_IDX,
                                                                                           ES_IDX)).withIdFn((node)->node.get("occurrenceID").textValue()));
    ...
    pipeline.apply(AvroIO.read(TemporalRecord.class).from(INPUT+"temporal/interpreted*.avro"))
      ...
      .apply(ElasticsearchIO.write()
               .withConnectionConfiguration(ElasticsearchIO.ConnectionConfiguration.create(ES_ADDRESSES,
                                                                                           ES_IDX,
                                                                                           ES_IDX)).withIdFn((node)->node.get("id").textValue()));
