...
public class MultimediaParser {
                  ...
                  extendedRecord
                      ...
                      .forEach(
                          recordsMap -> {
                            ...
                            Optional<TermWithValue> uriTermOpt =
                                getTermWithValueOfFirst(
                                    recordsMap, AcTerm.accessURI, DcTerm.identifier);
                            Optional<TermWithValue> linkTermOpt =
                                getTermWithValueOfFirst(
                                    recordsMap,
                                    DcTerm.references,
                            ...
                            if (Objects.isNull(uri) && Objects.isNull(link)) {
                              ...
                              uriTermOpt.ifPresent(
                                  termWithValue -> termsUsed.add(termWithValue.term));
                              linkTermOpt.ifPresent(
                                  termWithValue -> termsUsed.add(termWithValue.term));
                              issues.add(
                                  InterpretationIssue.of(
                                      IssueType.MULTIMEDIA_URI_INVALID, termsUsed));
                              return;
                            }
                            ...
                            parsedMultimediaMap.computeIfAbsent(
                                getPreferredIdentifier(atomicFields),
                                identifier ->
                                    ...
                                    ParsedMultimedia.newBuilder()
                                        ...
                                        .description(
                                            getValueOfFirst(
                                                recordsMap, DcTerm.description, AcTerm.caption))
                                        .license(
                                            getValueOfFirst(
                                                recordsMap, DcTerm.license, DcTerm.rights))
                                        .publisher(getTermValue(recordsMap, DcTerm.publisher))
                                        .contributor(getTermValue(recordsMap, DcTerm.contributor))
                                        .source(
                                            getValueOfFirst(
                                                recordsMap, DcTerm.source, AcTerm.derivedFrom))
          ...
          .forEach(
              uri -> {
                if (Objects.isNull(uri)) {
                  issues.add(
                      InterpretationIssue.of(
                          IssueType.MULTIMEDIA_URI_INVALID, DwcTerm.associatedMedia));
                  return;
                }

                // parse the atomic fields at once
                AtomicFields atomicFields =
                    parseAtomicFields(extendedRecord.getCoreTerms(), uri, null);
  ...
  private static AtomicFields parseAtomicFields(
    ...
    // get format
    String format =
        Optional.ofNullable(MEDIA_PARSER.parseMimeType(getTermValue(recordsMap, DcTerm.format)))
  ...
  private static Temporal parseCreatedDate(
      Map<String, String> recordsMap, List<InterpretationIssue> issues) {
  ...
  private static Optional<TermWithValue> getTermWithValueOfFirst(
      Map<String, String> record, Term... terms) {
    return Arrays.stream(terms)
        .filter(term -> record.containsKey(term.qualifiedName()))
