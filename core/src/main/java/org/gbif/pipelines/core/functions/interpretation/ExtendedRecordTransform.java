...
public class ExtendedRecordTransform extends PTransform<PCollection<ExtendedRecord>, PCollectionTuple> {
  ...
  public PCollectionTuple expand(PCollection<ExtendedRecord> input) {
    return input.apply("Interpreting record level terms", ParDo.of(mutate())
      .withOutputTags(INTERPRETED_EXTENDED_RECORD_TUPLE_TAG, TupleTagList.of(OCCURRENCE_ISSUE_TUPLE_TAG)));
  }

  private DoFn<ExtendedRecord,InterpretedExtendedRecord> mutate() {
    return new DoFn<ExtendedRecord, InterpretedExtendedRecord>() {
      ...
      public void processElement(ProcessContext context) {
        ...
        if (!validations.isEmpty()) {
          context.output(OCCURRENCE_ISSUE_TUPLE_TAG, org.gbif.pipelines.io.avro.OccurrenceIssue.newBuilder()
            .setId(extendedRecord.getId())
            .setIssues(validations).build());
        }
        context.output(INTERPRETED_EXTENDED_RECORD_TUPLE_TAG, interpretedExtendedRecord);
      }
    };
  }
