...
public class DataResourceNbnCache {
  ...
  public DataResourceNbn getDataResourceNbn(String uid) {
    ...
    return cache.computeIfAbsent(
        ...
        key -> {
          try {
            //  TODO NBN              String url = Config.registryUrl() +
            // "/accessControl/dataResourceNbn/" + uid+"?api_key=" + Config.collectoryApiKey();
            String url =
                "https://registry.legacy.nbnatlas.org/ws/accessControl/dataResourceNbn/"
                    + uid
                    + "?api_key=fd051578-4a55-4b4f-864d-c71923e2d029";
            ...
            try (Response response = client.newCall(request).execute()) {
              if (response.code() == 404) {
                ...
                System.out.println(
                    "Request failed with status code: " + response.code() + " api call:" + url);
                return null;
              } else {
                ...
                DataResourceNbn dataResourceNbn =
                    objectMapper.readValue(json, DataResourceNbn.class);
