...
/**
 * A temporary pipeline used for data migration. This uses a extract from the cassandra occ_uuid and
 * generates the <DATASET_ID>/1/identifiers/ala_uuid files in AVRO and distributes them for each
 * dataset into separate directories.
 *
 * <p>This class should not be part of the codebase in the long term and should be removed
 * eventually.
 *
 * <p>Note: this pipeline can currently only be ran with the DirectRunner due to issues with
 * SparkRunner logged here: https://jira.apache.org/jira/browse/BEAM-10100
 */
@Slf4j
...
public class MigrateUUIDPipeline {
  ...
  public static void run(BasePipelineOptions options) {
    ...
    PCollection<KV<String, ALAUUIDRecord>> records =
    ...
    // Group by dataResourceUID, keying on <dataResourceUID> to enable dynamic file writing
    // destinations
    // Write to /data/pipelines-data/<dataResourceUID>/...ala_uuid using the dynamicDestinations
    // capability of Beam
    records.apply(
        "Write avro file per dataset",
        FileIO.<String, KV<String, ALAUUIDRecord>>writeDynamic()
            .by(KV::getKey)
            .via(
                Contextful.fn(KV::getValue),
                Contextful.fn(x -> AvroIO.sink(ALAUUIDRecord.class).withCodec(BASE_CODEC)))
            ...
            .withNaming(
                key ->
                    defaultNaming(
                        key
                            + "/1/identifiers/"
                            + ALARecordTypes.ALA_UUID.toString().toLowerCase()
                            + "/interpret",
  ...
  static class StringToDatasetIDAvroRecordFcn extends DoFn<String, KV<String, ALAUUIDRecord>> {
    ...
    public void processElement(
        @Element String line, OutputReceiver<KV<String, ALAUUIDRecord>> out) {
      try {
